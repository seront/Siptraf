...
<c:if test="${!empty idLinea}">
    ...
    <c:forEach var="estParada" items="${gl.buscarEstacion(idLinea)}" begin="1" end="${gl.buscarEstacion(idLinea).size()-2}">
        ...
        <input type="number" class="campoFormulario estacionesDesc" id="${estParada.estacionPK.idPkEstacion}" title="Minutos de parada estandar en esta estacion" min="0" step="1"/>
    </c:forEach>        
</c:if>
